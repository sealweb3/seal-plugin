...
async function handleManagerButtonClick(button) {
    ...
    if (typeof window.ethereum !== 'undefined') {
        ...
        try {
            ...
            const actionType = button.getAttribute('data-action') || 'manager';

            console.log('Sendirver:', actionType);
            const response = await sendDataToManager(actionType, signature, userAddress, message);
  ...
  async function sendDataToManager(action, signature = '', userAddress = '', singMessage = '') {
    console.log('Sending to server:', action, signature, userAddress, singMessage);
    //const messageHash = ethers.hashMessage(message); 
    
    try {
        const response = await fetch('../seal/js/manager.php', { 
            method: 'POST',
