...
function get_program() {
    global $CFG;
    $cookieNameToken = 'seal-attestation-token';

    // Check and retrieve the token from cookies
    if (isset($_COOKIE[$cookieNameToken])) {
        $token = json_decode($_COOKIE[$cookieNameToken], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new moodle_exception('Invalid token in the cookie');
        }
    } else {
        throw new moodle_exception('Token not found in the cookie Umm');
    }
    ...
    $url = get_config('mod_seal', 'url') . '/profiles/getProgramsByProfileId/' . urlencode($profileid);
    //$url = get_config('mod_seal', 'url') . '/profiles/getProgramsByProfileId/0x07ed5f0cd1d6eae7abd9077f468fcbff61e7d4cbfc318fab54585fa14f951e89';
    ...
    $headers = [
        'Ngrok-Skip-Browser-Warning: true',
        'Accept: application/json',
        'Authorization: Bearer ' . $token['accessToken']  // Correct string concatenation for PHP
    ];
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    $response = curl_exec($ch);
    if ($response === false) {
        $errorMessage = 'Failed to fetch program from external API: ' . curl_error($ch);
        error_log($errorMessage);
        curl_close($ch);
        throw new moodle_exception($errorMessage);
    }
    ...
    if (!$response) {
        throw new moodle_exception('Invalid response from external API');
    }
